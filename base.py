...
class System:
    def __init__(self,master):
        ...
        # Top Frame
        top_frame = Frame(main_frame,bg="black", width= 900, height= 70, borderwidth = 2,relief= SUNKEN, padx= 20)
        top_frame.pack(side= TOP, fill= X)

        self.btn_add_member = Button(top_frame,bg="cadetblue",text="Add New Member", font="arial 12 bold",padx=10,command=self.new_member)
        self.btn_add_member.pack(side=LEFT)

        self.btn_add_book = Button(top_frame,bg="cadetblue",text="Add New Book", font="arial 12 bold",padx=10,command=self.new_book)
        self.btn_add_book.pack(side=LEFT)

        self.btn_issue_book = Button(top_frame,bg="cadetblue", text="Issue Book", font="arial 12 bold",padx=10, command = self.issue_book)
        self.btn_issue_book.pack(side=LEFT)


        # Centre Frame
        centre_frame = Frame(main_frame,bg="cadetblue", width = 900, height= 800, relief = RIDGE)
        centre_frame.pack(side= TOP)

        # Left Frame
        left_frame = Frame(centre_frame,bg="darkblue", width=600, height=700, relief=SUNKEN, borderwidth=2)
        ...
        # Management
        self.management_box = Listbox(self.tab1, width=40, height=30,bg="darkturquoise", font='times 12 bold')
        ...
        self.list_details = Listbox(self.tab1, width=80,height = 30,bg="darkcyan",font= 'times 12 bold')
...
def base():
    ...
    base_window.title("Mordenizing Libraries")
